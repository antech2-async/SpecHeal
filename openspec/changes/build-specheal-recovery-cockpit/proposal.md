...
- Menambahkan OpenSpec-guarded recovery pipeline yang memakai live OpenAI untuk verdict `HEAL`, `PRODUCT BUG`, `SPEC OUTDATED`, atau `NO_HEAL_NEEDED`.
- Menambahkan Jira integration yang otomatis membuat Jira issue untuk setiap run final, termasuk successful/no-heal, heal, product bug, spec outdated, dan run error.
...
- `jira-integration`: Mendefinisikan behavior publikasi otomatis ke Jira untuk semua status run, termasuk payload mapping, issue type mapping, publish status, failure handling, dan retry.
