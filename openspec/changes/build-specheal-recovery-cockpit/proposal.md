...
- Menambahkan Playwright runtime execution untuk menjalankan checkout test, mengambil failure evidence, memvalidasi candidate selector, menerapkan patch ke test file saat `HEAL`, dan melakukan rerun proof.
- Menambahkan OpenSpec-guarded recovery pipeline yang memakai live OpenAI `gpt-4o-mini` untuk failed-run verdict `HEAL`, `PRODUCT BUG`, atau `SPEC OUTDATED`, sementara successful baseline run diklasifikasikan sebagai `NO_HEAL_NEEDED`.
...
- Menambahkan PostgreSQL persistence untuk run history, evidence summary, base64 screenshot, AI trace, applied patch preview, validation/rerun result, dan Jira publish result.
- Menambahkan deployment readiness agar runtime product dapat berjalan sebagai satu app container plus PostgreSQL service di Kubernetes pada VPS hackathon.
...
- Playwright runtime, controlled test-file patching, dan browser dependency.
