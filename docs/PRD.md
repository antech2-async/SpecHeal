...
1. Menyediakan dashboard untuk menjalankan scenario recovery UI test tanpa perlu terminal.
2. Menggunakan live OpenAI untuk menganalisis failure evidence dan OpenSpec requirement.
3. Membedakan `HEAL`, `PRODUCT BUG`, `SPEC OUTDATED`, dan `NO_HEAL_NEEDED` secara akurat.
4. Membuktikan `HEAL` melalui browser validation dan rerun proof — bukan hanya asumsi AI.
5. Mempublikasikan Jira issue secara live untuk setiap hasil recovery yang membutuhkan action.
6. Menyimpan run history dan audit trail lengkap ke PostgreSQL.
7. Menyediakan full report yang dapat diaudit oleh judge, mentor, dan developer.
...
**Output:** Jira Task untuk update test/spec mapping.
